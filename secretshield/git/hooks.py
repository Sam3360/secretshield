...
def get_diff_files(ref: str) -> list[str]:
    """
    Return the relative paths of files changed relative to `ref` (e.g.
    "HEAD~1", "origin/main...HEAD") that still exist and were
    added/copied/modified -- suitable for "did my changes introduce a
    secret?" checks against the current working tree content.
    """
    try:
        result = subprocess.run(
