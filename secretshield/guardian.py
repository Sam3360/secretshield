...
# Module-level state tracking whether protection has been explicitly
# enabled (used by is_enabled()). The actual wrap/unwrap decision on
# each enable()/disable() call is made by inspecting sys.stdout/
# sys.stderr directly (see below) rather than trusting this flag alone.
_state: dict[str, Any] = {
    "active": False,
}
...
def enable() -> None:
    ...
    Always (re)wraps whatever ``sys.stdout``/``sys.stderr`` currently
    are, rather than relying solely on a single global "already active"
    flag to skip the work. That matters if something else reassigns
    those streams between calls -- a test framework's output capture,
    a user's own redirection, etc. -- so a later ``enable()`` correctly
    protects the stream actually in use *now*, instead of silently
    doing nothing because protection was already on for some earlier,
    now-replaced stream.
    """
...
def disable() -> None:
    ...
    Unwraps whatever ``sys.stdout``/``sys.stderr`` currently are (only
    if they're currently a guarded stream), rather than restoring a
    reference captured back whenever ``enable()`` first happened to run
    -- which could be stale if the streams were swapped out since then.
    """
    for name in ("stdout", "stderr"):
        stream = getattr(sys, name)
        if isinstance(stream, _GuardedStream):
            # Flush any buffered partial-line content (e.g. output with
            # no trailing newline yet) before unwrapping, so it isn't
            # silently lost.
            try:
                stream.flush()
            except Exception:
                pass
            setattr(sys, name, stream._wrapped)
