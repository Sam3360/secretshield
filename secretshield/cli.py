...
from .color import green, red, supports_color
...
def _plural(count: int, word: str) -> str:
    """`f"{count} {word}"`, pluralized properly -- "1 rule" / "2 rules" --
    instead of the awkward literal "rule(s)" regardless of count."""
...
def _run_scan(args: argparse.Namespace) -> int:
    ...
    from .custom_rules import load_custom_rules, scan_text_with_rules
    ...
    from .ignorefile import load_ignore_patterns

    exclude_patterns.extend(load_ignore_patterns(Path.cwd()))

    # Custom rules (.secretshield-rules.toml), if present, become
    # additional detectors alongside the built-in patterns -- no flag
    # required. A broken rules file never blocks the scan (valid rules
    # in the same file still load and run); it's surfaced as a single
    # warning pointing at `rules check` rather than silently ignored.
    custom_rules, custom_rule_errors = load_custom_rules(Path.cwd())
    if custom_rule_errors:
        print(
            f"secretshield: warning: {_plural(len(custom_rule_errors), 'invalid custom rule')} "
            f"in .secretshield-rules.toml -- run `secretshield rules "
            f"check` for details.",
            file=sys.stderr,
        )
    ...
    for display_path, text in _iter_scan_sources(args, target, include_patterns, exclude_patterns):
        ...
        if custom_rules:
            custom_matches = [
                m for m in scan_text_with_rules(text, custom_rules) if not _is_placeholder(m.value)
            ]
            file_matches = file_matches + custom_matches
        if file_matches:
    ...
    for finding in all_findings:
        ...
        print(f"{red(chr(0x2717), use_color)} {finding['file']}:{finding['line']}")
        print(f"  Potential secret: {label}")
        print(f"  Type: {category}\n")

    if ignored_by_baseline:
        print(
            f"{green(chr(0x2713), use_color)} "
            f"{ignored_by_baseline} existing finding(s) ignored by baseline"
        )

    if all_findings:
        print(f"{green(chr(0x2713), use_color)} {files_scanned} files scanned")
        print(f"{red(chr(0x2717), use_color)} {len(all_findings)} potential secret(s) found")
    else:
        print(f"{green(chr(0x2713), use_color)} {files_scanned} files scanned")
        print(f"{green(chr(0x2713), use_color)} No potential secrets found")
...
def _run_scan_staged(args: argparse.Namespace) -> int:
    ...
    if not findings:
        print(f"{green(chr(0x2713), use_color)} No secrets detected.")
        return 0

    for finding in findings:
        print(f"{red(chr(0x2717), use_color)} Potential secret detected\n")
...
def _cmd_install_hook(args: argparse.Namespace) -> int:
    ...
    if result.status == "error":
        ...
        return 1
    print(f"{green(chr(0x2713), supports_color(sys.stdout))} {result.message}")
    return 0
...
def _cmd_github_action(args: argparse.Namespace) -> int:
    ...
    if not written:
        ...
        return 1
    print(f"{green(chr(0x2713), supports_color(sys.stdout))} Created {path}")
    return 0
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    print(f"{green(chr(0x2713), use_color) if is_python_project else red(chr(0x2717), use_color)} "
          f"{'Detected Python project' if is_python_project else 'No Python project markers found'}")
    print(f"{green(chr(0x2713), use_color) if in_git_repo else red(chr(0x2717), use_color)} "
    ...
    if want_config:
        ...
        if existing is not None:
            print(f"{green(chr(0x2713), use_color)} secretshield.toml already exists, leaving it unchanged")
        else:
            config_path = project_root / "secretshield.toml"
            config_path.write_text(render_default_toml(), encoding="utf-8")
            print(f"{green(chr(0x2713), use_color)} Created {config_path.name}")
            created.append(config_path.name)

    if want_hook:
        result = install_hook()
        if result.status == "error":
            print(f"{red(chr(0x2717), use_color)} {result.message}")
        else:
            print(f"{green(chr(0x2713), use_color)} {result.message}")
            created.append("Git pre-commit hook")

    if want_actions:
        written, path = generate_workflow(project_root)
        if written:
            print(f"{green(chr(0x2713), use_color)} Created {path.relative_to(project_root)}")
            created.append(str(path.relative_to(project_root)))
        else:
            print(f"{green(chr(0x2713), use_color)} {path.relative_to(project_root)} already exists, leaving it unchanged")

    print("\nSecretShield is ready.")
    return 0


# --- `rules` / `rule create` commands -----------------------------------


def _cmd_rules_check(args: argparse.Namespace) -> int:
    from .custom_rules import load_custom_rules, rules_file_path

    use_color = supports_color(sys.stdout)
    path = rules_file_path(Path.cwd())

    if not path.exists():
        print(f"No {path.name} found.")
        return 0

    rules, errors = load_custom_rules(Path.cwd())

    print("SecretShield Rules\n")

    if errors:
        for err in errors:
            if err.name:
                print(f'{red(chr(0x2717), use_color)} Invalid custom rule "{err.name}"')
                print(f"  {err.message}")
            else:
                label = f"rule #{err.rule_index + 1}" if err.rule_index is not None else path.name
                print(f"{red(chr(0x2717), use_color)} {label}: {err.message}")
        return 1

    print(f"{green(chr(0x2713), use_color)} {path.name}")
    print(f"{green(chr(0x2713), use_color)} {_plural(len(rules), 'rule')} loaded")
    print(f"{green(chr(0x2713), use_color)} All patterns are valid")
    return 0
...
def _cmd_rules_list(args: argparse.Namespace) -> int:
    ...
    if rules:
        print("SecretShield Custom Rules\n")
    ...
    if errors:
        use_color = supports_color(sys.stdout)
        print(
            f"\n{red(chr(0x2717), use_color)} {_plural(len(errors), 'rule')} failed to load "
            f"-- run `secretshield rules check` for details."
        )
    return 0


def _cmd_rules_no_subcommand(args: argparse.Namespace) -> int:
    # `secretshield rules` with no further action -> list.
    return _cmd_rules_list(args)


def _cmd_rule_no_subcommand(args: argparse.Namespace) -> int:
    print("secretshield rule: missing subcommand. Try `secretshield rule create`.", file=sys.stderr)
    return 1


def _cmd_rule_create(args: argparse.Namespace) -> int:
    import re as _re

    from .custom_rules import append_rule, rules_file_path
    ...
    if not interactive:
        print("secretshield rule create requires an interactive terminal.")
        print("No rule was created.")
        return 1

    print("SecretShield \u2014 Create custom rule\n")

    name = input("Rule name: ").strip()
    if not name:
        print("Rule name cannot be empty. No rule was created.", file=sys.stderr)
        return 1

    while True:
        pattern = input("Pattern: ").strip()
        if not pattern:
            print("Pattern cannot be empty.", file=sys.stderr)
            continue
        try:
            _re.compile(pattern)
    ...
    path = rules_file_path(Path.cwd())
    try:
        append_rule(path, name, pattern)
    except Exception as exc:
        print(f"\u2717 Could not write {path.name}: {exc}", file=sys.stderr)
        return 1

    print(f"{green(chr(0x2713), supports_color(sys.stdout))} Added rule to {path.name}")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    rules_parser = subparsers.add_parser(
        "rules",
        help="List or validate .secretshield-rules.toml custom detection rules.",
    )
    rules_subparsers = rules_parser.add_subparsers(dest="rules_action")
    rules_check_parser = rules_subparsers.add_parser(
        "check", help="Validate .secretshield-rules.toml (TOML syntax, required fields, regex validity)."
    )
    rules_check_parser.set_defaults(func=_cmd_rules_check)
    rules_parser.set_defaults(func=_cmd_rules_list)

    rule_parser = subparsers.add_parser(
        "rule", help="Create a custom detection rule."
    )
    rule_parser.set_defaults(func=_cmd_rule_no_subcommand)
    rule_subparsers = rule_parser.add_subparsers(dest="rule_action")
    rule_create_parser = rule_subparsers.add_parser(
        "create", help="Interactively create a new rule in .secretshield-rules.toml."
    )
    rule_create_parser.set_defaults(func=_cmd_rule_create)
