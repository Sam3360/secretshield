...
def _run_scan(args: argparse.Namespace) -> int:
    ...
    if custom_rule_errors:
        print(
            f"secretshield: warning: {len(custom_rule_errors)} invalid custom "
            f"rule(s) in .secretshield-rules.toml -- run `secretshield rules "
            f"check` for details.",
            file=sys.stderr,
        )
...
def _cmd_rules_check(args: argparse.Namespace) -> int:
    ...
    print(f"{green(chr(0x2713), use_color)} {len(rules)} rule(s) loaded")
...
def _cmd_rules_list(args: argparse.Namespace) -> int:
    ...
    if errors:
        use_color = supports_color(sys.stdout)
        print(
            f"\n{red(chr(0x2717), use_color)} {len(errors)} rule(s) failed to load "
