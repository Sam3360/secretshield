...
from ..custom_rules import scan_text_with_rules
from . import javascript as jsfix
...
def _find_safe_assignment_for(suffix: str, line_text: str, secret_value: str):
    """Dispatch to the right language's safe-assignment check. Returns
    None (ambiguous / unsupported) for anything not explicitly handled."""
    if suffix == ".py":
        return pyfix.find_safe_assignment(line_text, secret_value)
    if suffix in _JS_TS_EXTENSIONS:
        return jsfix.find_safe_assignment(line_text, secret_value)
    return None


def _build_replacement_line(suffix: str, safe, env_key: str) -> str:
    if suffix == ".py":
        return pyfix.build_getenv_line(safe.indent, safe.var_name, env_key, safe.quote)
    if suffix in _JS_TS_EXTENSIONS:
        return jsfix.build_getenv_line(safe, env_key, is_typescript=suffix in (".ts", ".tsx"))
    raise ValueError(f"unsupported suffix for Auto-Fix: {suffix}")
...
def run_autofix(
    ...
    `custom_rules` (optional): the same `.secretshield-rules.toml`
    rules used to build `findings_by_file`, if any. Needed here too so
    that the internal re-scan (used to get fresh, accurate line numbers
    after an `import os` insertion) can re-locate a match that only a
    custom rule -- not a built-in pattern -- would find.
    """
    summary = FixSummary()
    ...
    for relpath in sorted(findings_by_file.keys()):
        ...
        # assignment; if so (and, for Python, `import os` is missing)
        # insert it first.
        any_safe = False
        for m in matches:
            line_no = _line_at(source, m.start)
            if 1 <= line_no <= len(lines):
                if _find_safe_assignment_for(suffix, lines[line_no - 1], m.value):
                    any_safe = True
                    break

        if suffix == ".py" and any_safe and not pyfix.has_import_os(source):
            source = pyfix.insert_import_os(source)

        # Re-scan the (possibly import-adjusted) source fresh, so line
        # numbers reflect the current content exactly. Custom rules are
        # merged in too, so a match only a custom rule (not a built-in
        # pattern) would find isn't lost here.
        try:
            current_matches = detect(source, entropy_threshold=entropy_threshold)
            if custom_rules:
                current_matches = current_matches + scan_text_with_rules(source, custom_rules)
        except Exception:
            current_matches = []
        ...
        for m in matches:
            ...
            safe = _find_safe_assignment_for(suffix, lines[line_no - 1], current.value)
            if safe is None:
                print_func(
            ...
            new_line = _build_replacement_line(suffix, safe, key)
