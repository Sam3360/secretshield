...
def suggest_env_var_name(var_name: str) -> str:
    """
    Turn an identifier into a reasonable environment variable name:
    `api_key` -> `API_KEY`, `OPENAI_API_KEY` -> unchanged, and (since
    this heuristic is shared with JS/TS Auto-Fix, where camelCase is
    the norm) `apiKey` -> `API_KEY`, `myAPIKey` -> `MY_API_KEY`.
    """
    # Split camelCase/PascalCase word boundaries before uppercasing, so
    # a run-together camelCase name doesn't collapse into one word.
    with_boundaries = re.sub(r"(?<=[a-z0-9])(?=[A-Z])", "_", var_name)
    with_boundaries = re.sub(r"(?<=[A-Z])(?=[A-Z][a-z])", "_", with_boundaries)
    cleaned = re.sub(r"[^A-Za-z0-9]+", "_", with_boundaries).strip("_").upper()
...
SUPPORTED_FIX_EXTENSIONS = {".py", ".js", ".jsx", ".ts", ".tsx"}
