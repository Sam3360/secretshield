...
#### `--fix` — interactively move a secret into `.env`
...
**Supported for Auto-Fix: Python, JavaScript, TypeScript (and JSX/TSX).**
Each language gets a genuinely safe check, not a guess — Python via its
own `ast` module, JS/TS via a deliberately narrow parser built for this
purpose. Both reject anything less than a fully unambiguous single
assignment: a value inside a dict/object literal, an f-string or
template-literal interpolation, a function-call argument, or more than
one variable on the same line is reported but left untouched. On
TypeScript, the rewritten line gets a `!` non-null assertion so it
stays type-compatible with any existing type annotation. Other
languages are still fully *scanned* — only the automatic rewrite is
limited to these three, per SecretShield's "detect aggressively, modify
conservatively" rule. `--fix` only prompts in a real interactive
terminal; it makes no changes in CI or any other non-interactive
environment.
...
│   ├── python.py                                       # Python: ast-based safety check
│   └── javascript.py                                   # JS/TS: narrow safety check (v0.6.0)
├── git/                                              # pre-commit hook
