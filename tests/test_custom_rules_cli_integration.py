...
def test_rules_check_success(tmp_path, monkeypatch, capsys):
    ...
    assert exit_code == 0
    out = capsys.readouterr().out
    assert "2 rules loaded" in out
    assert "All patterns are valid" in out


# --- Regression tests: singular/plural grammar in rule-count output --------
# (0.7.1: these counts used to always print the literal "rule(s)"
# regardless of count, e.g. "1 rule(s) loaded" -- fixed to pluralize
# properly.)


def test_rules_check_singular_grammar(tmp_path, monkeypatch, capsys):
    monkeypatch.chdir(tmp_path)
    (tmp_path / ".secretshield-rules.toml").write_text(
        '[[rules]]\nname = "Only One"\npattern = "ONE_[0-9]{4}"\n'
    )
    main(["rules", "check"])
    out = capsys.readouterr().out
    assert "1 rule loaded" in out
    assert "rule(s)" not in out


def test_rules_check_plural_grammar(tmp_path, monkeypatch, capsys):
    monkeypatch.chdir(tmp_path)
    (tmp_path / ".secretshield-rules.toml").write_text(
        '[[rules]]\nname = "A"\npattern = "AAA_[0-9]{4}"\n'
        '[[rules]]\nname = "B"\npattern = "BBB_[0-9]{4}"\n'
...
def test_rules_list_singular_error_grammar(tmp_path, monkeypatch, capsys):
    monkeypatch.chdir(tmp_path)
    (tmp_path / ".secretshield-rules.toml").write_text(
        '[[rules]]\nname = "Broken"\npattern = "["\n'
    )
    main(["rules"])
    out = capsys.readouterr().out
    assert "1 rule failed to load" in out
    assert "rule(s)" not in out


def test_scan_warning_singular_grammar(tmp_path, monkeypatch, capsys):
    monkeypatch.chdir(tmp_path)
    (tmp_path / ".secretshield-rules.toml").write_text(
        '[[rules]]\nname = "Broken"\npattern = "["\n'
    )
    ...
    main(["scan", "."])
    err = capsys.readouterr().err
    assert "1 invalid custom rule in" in err
    assert "rule(s)" not in err
